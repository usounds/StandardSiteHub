...
import { TextInput, Textarea, Button, Stack, FileInput, Group, Image, Text, Switch, Paper, Divider } from '@mantine/core';
    ...
    return (
        <form onSubmit={form.onSubmit(onSubmit)}>
            <Paper withBorder className="app-panel" p={{ base: 'lg', sm: 'xl' }}>
                <Stack gap="lg">
                    <Stack gap={4}>
                        <Text fw={800}>{mode === 'create' ? t('title') : t('title')}</Text>
                        <Text size="sm" c="dimmed">{t('show_in_discover_description')}</Text>
                    </Stack>
                    <Divider />
                    <Group align="end">
                        <TextInput
                    ...
                    {ogpIconPreview && (
                        <Group className="app-muted-panel" p="sm" style={{ borderRadius: 'var(--mantine-radius-md)' }}>
